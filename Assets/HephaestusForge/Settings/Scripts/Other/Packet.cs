        ...
        public sealed class Packet : IDisposable
        {
            ...
            public string ReadString(bool moveReadPos = true)
            {
                try
                {
                    ...
                    string value = Encoding.UTF8.GetString(_readableBuffer, _readPos, length); // Convert the bytes to a string
            ...
            public void Write(SimpleResolution resolution)
            {
                Write(resolution._Width);
                Write(resolution._Height);
            }

            public SimpleResolution ReadSimpleResolution(bool moveReadPos = true)
            {
                return new SimpleResolution() { _Width = ReadInt(moveReadPos), _Height = ReadInt(moveReadPos) };
            }

            public void Write(SettingsData settingsData)
            {
                Write(settingsData._FullScreen);
                Write(settingsData._MasterVol);
                Write(settingsData._MusicVol);
                Write(settingsData._SFXVol);
                Write(settingsData._Resolution.Value);
                Write((int)settingsData._GraphicsQuality.Value);
                Write((int)settingsData._Language);
            }

            public SettingsData ReadSettingsData(bool moveReadPos = true)
            {
                var data = new SettingsData();
                data._FullScreen = new BooleanReference();
                data._MasterVol = new FloatReference();
                data._MusicVol = new FloatReference();
                data._SFXVol = new FloatReference();
                data._Resolution = new SimpleResolutionReference();
                data._GraphicsQuality = new QualitySettingValuesReference();

                data._FullScreen.Value = ReadBool(moveReadPos);
                data._MasterVol.Value = ReadFloat(moveReadPos);
                data._MusicVol.Value = ReadFloat(moveReadPos);
                data._SFXVol.Value = ReadFloat(moveReadPos);
                data._Resolution.Value = ReadSimpleResolution(moveReadPos);
                data._GraphicsQuality.Value = (QualitySettingValues)ReadInt(moveReadPos);
                data._Language = (Languages)ReadInt(moveReadPos);
